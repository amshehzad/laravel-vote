...
trait Votable
{
    ...
    public function hasBeenVotedByAuth(): bool
    {
        if (!auth()->check()) {
            return false;
        }

        return $this->hasBeenVotedBy(auth()->user());
    }

    public function hasBeenUpvotedByAuth(): bool
    {
        if (!auth()->check()) {
            return false;
        }

        return $this->hasBeenUpvotedBy(auth()->user());
    }

    public function hasBeenDownvotedByAuth(): bool
    {
        if (!auth()->check()) {
            return false;
        }

        return $this->hasBeenDownvotedBy(auth()->user());
    }
